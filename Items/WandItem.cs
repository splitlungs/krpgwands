...
namespace krpgwands
{
    ...
    public class WandItem : Item
    {
        ...
        public override void OnHeldInteractStop(float secondsUsed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel)
        {
            ...
            if (power > 0) entitymagicmissile.chilling = power;
            power = slot.Itemstack.Attributes.GetInt("igniting", 0);
            if (power > 0) entitymagicmissile.ignite = power;
            power = slot.Itemstack.Attributes.GetInt("lightning", 0);
            if (power > 0) entitymagicmissile.lightning = power;
            power = slot.Itemstack.Attributes.GetInt("pit", 0);
            if (power > 0) entitymagicmissile.pit = power;
